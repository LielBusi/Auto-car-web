...
type WorldInfo = {
  graph: any; // adjust with GraphInfo if exists
  roadWidth: number;
...
export class World {
  ...
  #updateLights() {
    ...
    const controlCenters: (Point & { lights: Light[]; ticks?: number })[] = [];

    for (const light of lights) {
      const point = getNearestPoint(light.center, this.#getIntersections());
      if (!point) continue; // <-- handle null here

      let controlCenter = controlCenters.find((c) => c.equals(point));
      if (!controlCenter) {
        controlCenter = new Point(point.x, point.y) as Point & {
          lights: Light[];
        };
    ...
    for (const center of controlCenters) {
      const cTick = tick % (center.ticks ?? 1);
